...
using static NoFog.Patches.ClientChatPatch;
    ...
    internal class BoxContentPanel : UEPanel
    {
        ...
        private void SendUpdateCommand()
        {
            ...
            if (string.IsNullOrEmpty(_boxName))
                return;
            MessageService.QueueMessage(string.Format(NoFog.Config.Settings.BCCOM_SWITCHBOX, _boxName));
            ...
            t.Elapsed += (_, _) =>
            {
                try
                {
                    MessageService.QueueMessage(NoFog.Config.Settings.BCCOM_BOXCONTENT);
                    t.Dispose();
                }
                finally
                {
        ...
        {
            EnableAllButtons(false);
            MessageService.QueueMessage(NoFog.Config.Settings.BCCOM_UNBINDFAM);
            ...
            t.Elapsed += (_, _) =>
            {
                try
                {
                    MessageService.QueueMessage(string.Format(NoFog.Config.Settings.BCCOM_BINDFAM, number));
                    t.Dispose();
                }
                finally
                {
